...
{/* Status Message - Shows admin note if exists, otherwise default */}
...
{transaction.status === 'approved' && (
  ...
  >
    {transaction.adminNotes ? (
      <>
        <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--color-text-primary)', marginBottom: 4 }}>
          Important Note
        </div>
        <div style={{ fontSize: 13, color: 'var(--color-text-primary)' }}>
          {transaction.adminNotes}
...
{transaction.status === 'rejected' && (
  <div
    style={{
      background: transaction.adminNotes ? 'rgba(239,68,68,0.08)' : 'rgba(239,68,68,0.08)',
      borderRadius: 12,
      padding: 12,
      textAlign: 'left',
      marginBottom: 16,
      border: transaction.adminNotes ? '1px solid rgba(239,68,68,0.2)' : 'none',
    }}
  >
    {transaction.adminNotes ? (
      <>
